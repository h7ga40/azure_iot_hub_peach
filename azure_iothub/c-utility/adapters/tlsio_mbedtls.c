...
static void mbedtls_init(TLS_IO_INSTANCE *tls_io_instance)
{
    ...
    if (tls_io_instance->tls_status != TLS_STATE_INITIALIZED)
    {
        ...
        mbedtls_ssl_conf_renegotiation(&tls_io_instance->config, MBEDTLS_SSL_RENEGOTIATION_ENABLED);
#endif
...
void tlsio_mbedtls_dowork(CONCRETE_IO_HANDLE tls_io)
{
    if (tls_io != NULL)
    {
        ...
        else if (tls_io_instance->tlsio_state == TLSIO_STATE_IN_HANDSHAKE || tls_io_instance->tlsio_state == TLSIO_STATE_OPEN)
        {
